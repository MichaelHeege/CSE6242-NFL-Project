...
TEAM_RANKS_PATHS = [
  BASE_DIR / 'data' / 'team_ranks.csv',
  BASE_DIR.parent / 'team_ranks.csv'
]

team_ranks_path = next((path for path in TEAM_RANKS_PATHS if path.exists()), None)
if team_ranks_path is None:
  raise FileNotFoundError("Could not find team_ranks.csv in Interface/data or the project root.")

team_ranks_df = pd.read_csv(team_ranks_path)
